...
public:
    ...
    size_t Get(TAirport airport) const {
        return airports_counter[(size_t)airport];
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        ++airports_counter[(size_t)airport];
    }
    ...
    void EraseAll(TAirport airport) {
        airports_counter[(size_t)airport] = 0;
    }

    using Item = size_t;//pair<TAirport, size_t>;
    using Items = array<std::size_t, (size_t)TAirport::Last_>;

    // получить некоторый объект, по которому можно проитерироваться,
    // получив набор объектов типа Item - пар (аэропорт, количество),
    // упорядоченных по аэропорту
    Items GetItems() const {
        return airports_counter;
    }

private:
    Items airports_counter{0};
};
...
#define ASSERT_EQUAL_ITEM(idx, expected_enum, expected_count) \
  do { \
    ASSERT_EQUAL(idx, static_cast<size_t>(MoscowAirport::expected_enum)); \
    ASSERT_EQUAL(items[idx], expected_count); \
  } while (false)                                                              \
//  do { \
...
void TestMostPopularAirport() {
    ...
    for (int day = 0; day < days_to_explore; ++day) {
        ...
        most_popular[day] = (SmallCountryAirports)(max_element(begin(items), end(items), [](auto lhs, auto rhs) {
            return lhs < rhs;}) - &items[0]);
    }
