...
public:
    ...
    int FindNearestFinish(int start, int finish) const {
        ...
        auto lowerb_it = reachable_stations.lower_bound(finish);

        if (lowerb_it != reachable_stations.end())
            result = std::min(result, abs(finish - *lowerb_it));
        if (lowerb_it != reachable_stations.begin())
            result = std::min(result, abs(finish - *std::prev(lowerb_it)));
